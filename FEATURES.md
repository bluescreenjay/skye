...
| **Google Calendar** | `calendar_create_event`, `calendar_list_events` | OAuth (the owner's Google account, **owner only**, and its authorization must include Calendar). An event goes on the person's own calendar for them alone: **no guests, no invitations, nobody notified**, and what will be created is shown on the button first. Listing shows title and times to the owner only and never reaches the AI |
...
- An MCP client module that connects to the seven configured servers (stdio and/or HTTP/SSE as each server requires), lists tools, maps them into OpenAI-compatible `tools` for VT ARC / Gemini, executes `tool_calls`, and records results on `ActionRun`
...
- Credential wiring via env / per-user secrets for the seven SaaS integrations; missing credentials → tool excluded from suggestions (and a clear “connect X” if the user somehow invokes it), not a crash
...
**Done when:** On an expanded Home card, the product shows a short, context-specific set of action buttons (not the full catalog). Those buttons can (1) write/export a summary (md + PDF), (2) open related tabs and Google searches in Chrome, and (3) with credentials configured, push or create something real in each of GitHub, Notion, Slack, Jira, Drive, Gmail (a draft; a send only after the person confirms the exact message), and Google Calendar (an event for the owner alone, no guests) when suggested—each run saved and visible after reload. Changing workspace context (or refresh) changes which buttons appear. All catalog tools exist behind the registry and are covered by fake-executor tests.
...
MCP-backed tools for seven integrations — implement at least: GitHub (create_issue, create_gist, search, comment_on_issue); Notion (create_page, append_blocks, search); Slack (post_message, upload_snippet); Jira (create_issue, search, add_comment); Google Drive (upload_markdown, create_doc_from_summary, get_share_link); Gmail (create_draft, send_message, search_messages — sending needs a separate confirm of the exact message); Google Calendar (create_event, list_events — owner only, no guests or invitations). Credentials from env or per-user secrets; missing auth fails clearly and keeps those tools out of suggestions.

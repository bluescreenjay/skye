...
- **`GEMINI_API_KEY` in the repo-root `.env`** (from Google AI Studio). Needed only for the live steps below; the automated checks use a fake model.
- Optionally `GEMINI_MODEL` and `CLUSTER_CONFIDENCE_BAR` (defaults in [contracts/model.md](./contracts/model.md)).
...
```bash
curl -s "https://generativelanguage.googleapis.com/v1beta/models?pageSize=100" \
  -H "x-goog-api-key: $GEMINI_API_KEY" | python3 -c 'import sys,json;[print(m["name"]) for m in json.load(sys.stdin)["models"] if "generateContent" in m.get("supportedGenerationMethods",[])]'
```

Check that the default `gemini-3.5-flash-lite` (or your `GEMINI_MODEL`) is listed. Listed does not mean callable (`gemini-2.5-flash` is listed but returns 404 for new users), so the first live run in V1 is the real check.
...
Stop the server, clear or break `GEMINI_API_KEY` (or set an invalid one), restart, run clustering. Expect `503 model_unconfigured` (no key) or `502 model_error` (bad key), and `/api/overview` identical to before. Restore the key afterwards.
...
- `GEMINI_API_KEY` or a real device token is committed

...
- Open item for the plan: the existing `Message` record and `messages` table have no way to mark a reply as unfinished (roles are only `user`, `assistant`, `system`). The plan must decide how to record that, or avoid the need, without breaking feature 001's shared type.

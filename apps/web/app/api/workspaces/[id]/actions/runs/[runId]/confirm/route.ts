...
import { confirmSend } from "@/src/actions/confirm";
...
export async function POST(request: Request, context: Context) {
  ...
  try {
    ...
    const to = body && typeof body === "object" && !Array.isArray(body) ? (body as { to?: unknown }).to : undefined;
    if (typeof to !== "string") {
      const err = invalidBody();
      return json({ error: err.message, code: err.code }, err.status);
    }
    const run = await confirmSend(user!.id, workspace.id, runId, to);
    return json({ run });
  } catch (caught) {
    ...
    console.error("confirm failed:", caught instanceof Error ? caught.name : "unknown");
